...
gera_individuo(ListaTarefas,NumT,[G|Resto]):- NumTemp is NumT + 1, % To use with random
	    random(1,NumTemp,N),
...
bsort([X],[X]):-!.
bsort([X|Xs],Ys):- bsort(Xs,Zs), btroca([X|Zs],Ys).


btroca([X],[X]):-!.

btroca([X*VX,Y*VY|L1],[Y*VY|L2]):- VX>VY,!, btroca([X*VX|L1],L2).

btroca([X|L1],[X|L2]):-btroca(L1,L2).


gera_geracao(G,G,Pop):-!, write('Geracao '), write(G), 
    write(':'), nl, write(Pop), nl.

gera_geracao(N,G,Pop):- write('Geracao '), write(N), 
    write(':'), nl, write(Pop), nl,
...
rotate_right(L,K,L1):- tarefas(N), T is N - K, rr(T,L,L1).
...
elimina([X|R1],L,[X|R2]):- not(member(X,L)),!, elimina(R1,L,R2).
...
insere1(X,1,L,[X|L]):-!.
insere1(X,N,[Y|L],[Y|L1]):- N1 is N-1, insere1(X,N1,L,L1).
